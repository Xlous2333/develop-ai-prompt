...
### 语言偏好
- **主要语言**: 简体中文 (zh-CN)
- **技术术语**: 优先使用英文原文，必要时提供中文解释
- **代码注释**: 使用英文或中文，保持项目一致性
...
- **铁律**: 任何代码实现之前必须先有对应文档
- **文档类型**: 需求文档、设计文档、API文档、架构决策记录(ADR)
- **文档更新**: 代码变更时同步更新文档
- **禁止行为**: 直接编写实现代码而不先完成设计文档

### 2. 明确确认 (Explicit Confirmation)
- **用户确认**: 关键决策点必须等待用户明确确认
- **假设禁止**: 绝不假设用户意图，有疑问立即询问
- **确认点**: 需求理解、架构设计、任务分解、质量评估
- **确认记录**: 在文档中明确记录用户的确认

### 3. 范围控制 (Scope Management)
- **明确边界**: 每个任务必须有清晰的"做什么"和"不做什么"
- **范围蔓延警惕**: 发现任何超出范围的讨论立即提醒
- **变更流程**: 范围变更必须走正式流程并更新文档
- **禁止词汇**: 避免"顺便"、"也可以"、"不如我们"等容易导致范围蔓延的表述
...
### 5. 可追溯性 (Traceability)
- **追溯链**: 维护从需求→设计→任务→代码→测试的完整链路
- **标识使用**: 使用 REQ-XXX, DES-XXX, TASK-XXX 等标识
- **代码追溯**: 在代码注释中包含追溯标识
- **影响分析**: 变更时分析影响范围
...
- **选择模式**: 根据任务类型选择最合适的专业模式
  ...
  - 架构设计 → `architect` 模式
  - 问题调试 → `debug` 模式
  - 测试编写 → `jest-test-engineer` 模式
  - 文档编写 → `documentation-writer` 模式
